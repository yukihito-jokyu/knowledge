...
複数Featureについて `feature_design -> design_review -> human_design_review -> task_breakdown` を繰り返す。
...
6. current feature の人間による詳細設計レビュー
7. current feature の task-breakdown
...
reviewが `pass` 以外なら人間による詳細設計レビューおよびTask Breakdownへ進めない。`return_to_feature_design` はFeature Designへ、`human_decision_required` は人間Decisionへ差し戻す。

### 5. 人間による詳細設計レビューを待つ

独立レビューが `pass` になったら、詳細設計、補助資料、`design-review.md` を利用者が確認できる機会を明示的に設ける。この時点ではTaskを作成しない。
