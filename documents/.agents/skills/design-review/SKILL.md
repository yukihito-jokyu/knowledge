...
対象Featureの詳細設計を、設計者とは独立した視点で正規ソースへ照合し、利用者レビューへ進める品質かを判定する。実装handoffは、独立レビューの `pass` に加えて利用者による詳細設計の明示承認後にのみ可能である。
...
2. 承認済みDecision
3. `docs/requirements/**`
4. `docs/planning/**` と `docs/design/**`
5. `docs/features/{feature_id}/requirements.md`、`design.md`、`design/**`
...
`design-review.md` に対象・正規ソース一覧・根拠トレーサビリティ表・資料間整合・総合判定・差し戻し事項・次のゲートを記録する。総合判定は `pass`、`return_to_feature_design`、`human_decision_required` のいずれかとする。`pass` の次のゲートは「利用者による詳細設計レビュー」であり、handoff可ではない。
