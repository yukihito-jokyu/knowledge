...
Concept または Scope を条件に、時点情報を持つ現行 Assertion を取得する。候補の新しさ・正しさ・優先度は判定しない。
...
| 出力 | `data.results[].temporal.valid_from`、`data.results[].temporal.valid_until`、`data.results[].temporal.observed_at`、`data.results[].temporal.last_verified` | RFC 3339 UTC stringまたは`null`、temporalに各1回 | 有効開始・終了、観測、最終確認の時刻。 |
...
```mermaid
flowchart TD
  A["名前付きoption入力"] --> B{"selector は有効か"}
  B -- はい --> C["Concept・Scope・時点情報を照会"]
...
```sql
-- SQL-01: Concept / Scope 条件をすべて満たし、時点情報を持つ現行 Assertion を取得する。
...
Concept / Alias の join で同一 Assertion が複数行に一致しても、`DISTINCT` により各 Assertion を1件だけ返す。`scope` の複数条件は、指定された key/value のすべてを現行 revision が持つ場合に一致する（AND集合）。
