...
| Evidence Temporal migration・既存CLI read JSON | pass | DEC-FEAT-020により非破壊migrationと既存`get-evidence`の任意`temporal`表示を追加し、互換性を検証する。 |
...
| --- | --- | --- |
| `testdata/fixtures/acceptance/knowledge-quality/` | 作成 | A〜J／Xの固定入力、隔離Store seed、期待oracle、期待Store差分を保持する。 |
| `test/integration/` | 作成・変更 | 実`knowledge` binary、隔離SQLite Store、stdout/stderr、exit code、履歴・差分をプロセス境界で観測する。DEC-FEAT-021の明示起動Runtime launcherもここに置く。 |
| `internal/persistence/sqlite/` | 変更 | DEC-FEAT-019の`integrationtest`限定同期gateと、DEC-FEAT-020のEvidence Temporal migration・Store readを提供する。 |
| `cmd/knowledge/` | 変更 | DEC-FEAT-020に従い、既存`get-evidence`のEvidenceへ任意`temporal`を表示する。 |

新operation、公開設定、既存Workflowの業務規則は変更しない。DEC-FEAT-019／020に限り、`internal/persistence/sqlite/`のtest gate・Evidence Temporal migrationと、`cmd/knowledge/`の既存`get-evidence`任意`temporal`表示を変更できる。
...
- **対象外／注記:** FixtureのJSON入れ子、test helper、実行コードの構造はImplementation領域が決める。DEC-FEAT-020以外の製品公開入力・出力・保存形式は追加しない。
...
- **関連要件:** REQ-021、BR-002、BR-003、BR-009、NFR-002〜006、DEC-FEAT-016〜020。
...
- **受入条件:**
  ...
  - 技術失敗Xでは`storage_error`、中断XではDEC-FEAT-019のtest-only同期gateにより`search-text`開始後・response開始前の無出力exit 130、Trace停止位置、Assessment不在、後続層未実行を観測できる。成功結論・推奨へ変換しない。
  - 検索不要のF／GにSearch Traceを要求せず、対象外層の未実行を合格へ読み替えない。
- **依存:** TASK-005-01。
- **対象外／注記:** Knowledge SearchおよびReading Valueの業務規則、Reading Value Skillは変更しない。DEC-FEAT-020以外の公開CLI／JSON、DB schemaは変更しない。
...
- **依存:** TASK-005-01。
- **対象外／注記:** 新CLI operation、意味検索、補償削除、再実行ledger、既存Workflowの判断規則変更は行わない。Evidence Temporalの読出しはDEC-FEAT-020に従う。

## TASK-005-04: プロセス境界の横断受入suiteと診断を完成する

- **目的:** 同じcase IDでFixture入力から各層の観測までを連結し、実CLIプロセス境界、Store差分、最初の不一致層、未実行層を再現可能に報告する。
- **関連要件:** REQ-021、BR-002、BR-003、BR-005、BR-008、BR-009、NFR-001〜006、DEC-FEAT-016、DEC-FEAT-019、DEC-FEAT-021。
...
- **受入条件:**
  ...
  - 公開CLIのargv、stdout/stderr、JSON、exit codeを観測対象として保持する。DEC-FEAT-020のEvidence Temporal migrationと既存`get-evidence`の任意`temporal`を実プロセスで検証し、その他のoperation・JSON・公開設定は変更しない。DEC-FEAT-019の同期gateも維持する。
...
| --- | --- |
| 判定 | pass |
| reviewer | implementation-readiness-review（独立・読み取り専用） |
| 親リポジトリ俯瞰の範囲 | 親リポジトリrootのAGENTS、workflow資料、FEAT-005正規資料・DEC-FEAT-016〜020、依存Feature handoff、テスト領域、被観測Workflow、Knowledge CLI、既存Fixture、SQLite実装、検証コマンドを確認した。 |
| 根拠 | DEC-FEAT-019により、`search-text`の中断を既存`integrationtest`限定gateへ局所接続して実プロセスで再現できる。DEC-FEAT-020により、Evidence Temporalを非破壊migrationで保存し、既存`get-evidence`へ任意表示を追加できる。通常binaryの中断gateはno-opであり、既存値なしEvidenceのresponse形状、公開設定は不変である。 |
