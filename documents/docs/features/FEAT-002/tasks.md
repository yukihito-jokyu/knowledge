...
> **前提:** 利用者の「タスクを作成して」（2026-08-15）を、詳細設計の明示承認およびTask分解開始指示として記録した。独立設計レビューは `pass` である。
...
| AssessmentとTraceの成果物契約 | pass | [assessment-template.md](design/assessment-template.md)、[search-trace.md](design/search-trace.md) |
...
| ID | タイトル | 論理領域 | 依存 |
| --- | --- | --- | --- |
| TASK-002-01 | Claim反復探索と評価判定を実現する | application / domain | FEAT-001 implementation handoff |
| TASK-002-02 | Assessment・Search Traceと失敗伝播を実現する | interface / application | TASK-002-01 |
| TASK-002-03 | 固定Budget・状態分類・中断を横断検証する | verification | TASK-002-01, TASK-002-02 |

## TASK-002-01: Claim反復探索と評価判定を実現する

- **目的:** 一つの独立評価可能なClaimに対して、既存Knowledge CLIを有限に反復利用し、Evidenceに基づく一つのKnowledge Assessment状態へ到達できるようにする。
- **関連要件:** REQ-004、REQ-005、REQ-019〜021、BR-002、BR-003、BR-006、BR-007、BR-010、NFR-001〜003、NFR-005〜006、DEC-REQ-001、DEC-FEAT-008。
- **論理領域:** application / domain。
- **作業内容:** 直接字句探索から構成要素またはConcept、必要なRelation、Evidence、矛盾候補、時点差分へ限定拡張する。Evidence原文、Scope、時点、Claimとの意味対応を評価し、優先規則に従って7状態、Confidence、Known、Knowledge Gap、矛盾・時点差分を決定する。探索を停止する条件とBudgetを一実行内で一貫して制御する。
- **受入条件:**
...
- **対象外／注記:** Articleの取得・Claim分解・Reading Valueの決定、Semantic Search、CLI/DB公開契約の変更は対象外。正規根拠は[design.md](design.md)と[DEC-FEAT-008](decisions/DEC-FEAT-008.md)。

## TASK-002-02: Assessment・Search Traceと失敗伝播を実現する

- **目的:** 評価結果と診断過程を別々のMarkdown成果物として出力し、CLI技術失敗と利用者中断を評価結果から明確に分離する。
- **関連要件:** REQ-005、REQ-019〜021、BR-002、BR-010、NFR-001〜003、NFR-005、CON-002、CON-003、DEC-FEAT-008。
- **論理領域:** interface / application。
- **作業内容:** 状態、Confidence、Known、Knowledge Gap、Supporting Evidence、Relation、矛盾、時点差分、結論、Trace参照をAssessmentへ構成する。Traceには操作、最小限の入力要約、結果/Evidence ID、増分、Budget、停止理由を残す。既存CLIの成功JSON、既知error、exit 130・無出力を区別して呼出側へ伝播する。
- **受入条件:**
  - Assessmentは[assessment-template.md](design/assessment-template.md)の必須節を持ち、Traceへ全操作ログを重複させない。
...
- **関連要件:** REQ-004、REQ-005、REQ-019〜021、BR-002、BR-003、BR-006、BR-007、BR-010、NFR-001〜003、NFR-005〜006。
- **論理領域:** verification。
- **作業内容:** 空Store、完全一致、構成知識、導出可能、矛盾、旧知識、競合Evidence、EvidenceなしRelation、技術失敗、キャンセルを含むFixtureを用い、Assessment、Trace、Parentへの結果が設計どおりであることを検証する。Scenario A〜Jをまたぐ横断Fixture評価はFEAT-005の所有範囲とし、本Taskは個別Claim評価のoracleを提供する。
- **受入条件:**
