...
依存Issueは、正本上の直接leaf依存だけを着手判定へ含める。着手依存欄に記載された親Taskは、OPENかCLOSEDかを問わず進捗文脈として記録して判定から除外する。推移的依存は、正本から直接依存ではないと証明できる場合、非阻害差異として記録し判定から除外する。直接leaf依存もclosedだけでは不十分である。まず基準refを更新し、依存成果物が実際に含まれるかをGit履歴で確認する。Issueのhuman-progress領域に統合Commitが記録されていれば、そのCommitが基準refに含まれることも確認する。記録が空欄・未実施のままでも、依存Issueがclosedで、単一のOwner Pathに対する成果物Commitを基準ref上で特定できる場合は、そのCommitを着手Evidenceとして使用し、Issue記録の同期漏れを警告する。Issue記録だけを根拠に、Git上で統合済みの依存を未統合と判定しない。
...
- 依存Commitがない: 最新の基準ref上でOwner成果物Commitを検索する。成果物が含まれていればそのCommitをEvidenceとして続行し、Issue記録の同期漏れを警告する。成果物も確認できない場合だけ開始を止める。
